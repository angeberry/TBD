...
from networks import mnist_net as model
...
class env(object):
    ...
    def train(self, X_data, y_data, X_valid=None, y_valid=None, load=False, epochs=10, name='model'):
        if load:
        ...
        for epoch in range(epochs):
            ...
            for batch in range(n_batch):
                ...
                _= self.sess.run([self.train_op], feed_dict={self.x: X_data[start:end], self.y:y_data[start:end], self.training:True})
...
adv_fgsm = model.make_fgsm(X_test, epochs=12, eps=0.1)
adv_deepfool = model.make_deepfool(X_test)
adv_jsma = model.make_jsma(X_test)
model.evaluate(adv_fgsm, y_test)
model.evaluate(adv_deepfool, y_test)
model.evaluate(adv_jsma, y_test)
